...
namespace Pagination.Helpers
{
    public class PaginationHelper
    {
        public static PagedResponse<List<T>> CreatePagedReponse<T>(
            List<T> pagedData, Information information, PaginationFilter validFilter, int totalRecords,
            IUriService uriService, string route
            )
        {
            var respose = new PagedResponse<List<T>>(
                pagedData, information, validFilter.PageNumber, validFilter.PageSize
                );
